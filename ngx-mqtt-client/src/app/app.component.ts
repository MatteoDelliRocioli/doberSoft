...
export class AppComponent implements OnDestroy {
     ...
     * */
    subscribe(): void {
        this._mqttService.subscribeTo<Foo>('ab123wz/cars/1')
            .subscribe({
                next: (msg: SubscriptionGrant | Foo) => {
                    if (msg instanceof SubscriptionGrant) {
                        this.status.push('Subscribed to ab123wz/cars/1 topic!');
                    } else {
     ...
     */
    sendMsg(): void {
        this._mqttService.publishTo<Foo>('ab123wz/cars/1', {bar: 'foo'}).subscribe({
            next: () => {
     ...
     */
    unsubscribe(): void {
        this._mqttService.unsubscribeFrom('ab123wz/cars/1').subscribe({
            next: () => {
                this.status.push('Unsubscribe from ab123wz/cars/1 topic');
            },
            error: (error: Error) => {
                this.status.push(`Something went wrong: ${error.message}`);
            }
